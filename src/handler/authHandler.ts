...
const authHandler = new HyperExpress.Router();

authHandler.get("/auth/google", async (req, res) => {
...
authHandler.get("/auth/google/callback", async (req, res) => {
  try {
    ...
    res
      ...
      .redirect(`/auth/google/app?authJwt=${jwtToken}&success=false`);
  }
});

authHandler.get("/auth/google/app", async (req, res) => {
  try {
    res.send(
